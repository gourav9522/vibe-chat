...
TELEGRAM_API_URL = f"https://api.xyz/bot{BOT_TOKEN}" # (Note: Yahan code mein apne real URL ka dhyan rakhna, niche wala use kar lena)
        ...
        if not TARGET_CHAT_ID or chat_id == TARGET_CHAT_ID:
            ...
            if file_id and chat_id:
                async with httpx.AsyncClient() as client:
                    file_path_res = (await client.get(f"https://api.telegram.org/bot{BOT_TOKEN}/getFile?file_id={file_id}")).json()
